...
import 'plugins/logtrail6/css/main.css';

import template from './templates/index.html';

const app = uiModules.get('app/logtrail6', []);
...
app.controller('logtrail6', function ($scope, kbnUrl, $route, $routeParams,
  $window, $interval, $http, $document, $timeout, $location, $sce) {
  $scope.title = 'LogTrail6';
  ...
  function init() {
    ...
    $http.get(chrome.addBasePath('/logtrail6/config')).then(function (resp) {
      if (resp.data.ok) {
      ...
      if ($routeParams.l) {
        $scope.selectedLevel = $routeParams.l === 'All' ? null : $routeParams.l;
      }
      if ($routeParams.t) {
  ...
  **/
  function doSearch(rangeType,order,actions,timestamp) {
    ...
    $scope.errorMessage = null;
    return $http.post(chrome.addBasePath('/logtrail6/search'), request).then(function (resp) {
      if (resp.data.ok) {
  ...
  */
  ...
  $scope.onSettingsChange = function () {
    ...
    $scope.selectedLevel = null; //all systems
    $scope.hosts = null;
    $scope.level = null;
    $scope.errorMessage = null;
  ...
  $scope.onLevelSelected = function (level) {
    var levelField = selectedIndexConfig.fields.mapping.level;
    angular.element('#level-picker').addClass('ng-hide');
    if (level === '*') {
      $scope.selectedLevel = null;
    } else {
      $scope.selectedLevel = level;
    }
    //$scope.userSearchText =  levelField  + ':"' + level + '"';
    $scope.onSearchClick();
  };
  ...
  function setupHostsList() {
    ...
    return new Promise((resolve, reject) => {
      $http.post(chrome.addBasePath('/logtrail6/hosts'),params).then(function (resp) {
        if (resp.data.ok) {
          $scope.hosts = [];
...
uiModules.get('app/logtrail6').directive('onLastRepeat', function () {
...
uiModules.get('app/logtrail6').directive('clickOutside', function ($document) {
  return {
    restrict: 'A',
    scope: false,
...
uiModules.get('app/logtrail6').filter('ansiToHtml', function ($sce) {
...
uiModules.get('app/logtrail6').directive('compileTemplate', function ($compile, $parse) {
  return {
