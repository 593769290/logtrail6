...
function convertToClientFormat(selectedConfig, esResponse) {
  ...
  for (let i = 0; i < hits.length; i++) {
    ...
    event.level = get(source, selectedConfig.fields.mapping.level);
...
module.exports = function (server) {
  ...
  server.route({
    ...
    handler: function (request, reply) {
	  ...
	  //If level is present then term query.
      if (request.payload.level != null ) {
        var termQuery = {
          term : {
          }
        };
        var levelField = selectedConfig.fields.mapping.level;
        let keywordSuffix = selectedConfig.fields.keyword_suffix;
        if (keywordSuffix == undefined) {
          levelField += ('.keyword');
        } else if (keywordSuffix.length > 0) {
          levelField += ('.' + keywordSuffix);
        }
        termQuery.term[levelField] = request.payload.level;
        searchRequest.body.query.bool.filter.bool.must.push(termQuery);
      }
...
function loadConfig(server) {
  return new Promise((resolve, reject) => {
    ...
    callWithInternalUser('get',request).then(function (resp) {
      ...
      var config = require('../../logtrail6.json');
      resolve(config);
    });
  });
}
